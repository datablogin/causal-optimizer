...
| Raw Coverage | `2022-01-01 06:00:00 UTC` to `2024-12-31 22:00:00 UTC` |
...
| Prepared File SHA256 | `1f07da692938837f8a74ef352a92f3a3dbd5093f1e0c6a90825a644eb5c46039` |
...
1. ERCOT timestamps were treated as US/Central market time (CST/CDT), then converted to UTC. The original "Hour Ending" convention was converted to interval-start: each timestamp represents the start of the hourly interval (1 hour subtracted from the raw hour-ending value). This matches the NORTH_C benchmark convention and enables valid cross-benchmark comparison.
...
6. Calendar features (hour_of_day, day_of_week, is_holiday) were derived from the interval-start time in ERCOT local market time (US/Central) before converting timestamps to UTC for storage.
...
|-----------|------|-------|-----|
| Train | `15,778` | `2022-01-01 06:00:00 UTC` | `2023-10-20 15:00:00 UTC` |
| Validation | `5,259` | `2023-10-20 16:00:00 UTC` | `2024-05-26 18:00:00 UTC` |
| Test | `5,260` | `2024-05-26 19:00:00 UTC` | `2024-12-31 22:00:00 UTC` |
...
|----------|-----------|-----------|-----------|
| random | `105.1439 +/- 0.3631` | `105.2017 +/- 0.3039` | `105.2081 +/- 0.2395` |
| surrogate_only | `105.8447 +/- 0.0034` | `105.8447 +/- 0.0034` | `105.5848 +/- 0.1515` |
| causal | `105.8447 +/- 0.0034` | `105.8447 +/- 0.0034` | `105.5848 +/- 0.1515` |

### Validation MAE (mean +/- std across seeds)

| Strategy | Budget 20 | Budget 40 | Budget 80 |
|----------|-----------|-----------|-----------|
| random | `90.2699 +/- 0.0559` | `90.2469 +/- 0.0478` | `90.1998 +/- 0.0415` |
| surrogate_only | `90.5174 +/- 0.0696` | `90.5174 +/- 0.0696` | `90.2498 +/- 0.1326` |
| causal | `90.5174 +/- 0.0696` | `90.5174 +/- 0.0696` | `90.2498 +/- 0.1326` |

### Validation-Test Gap (mean +/- std across seeds)

| Strategy | Budget 20 | Budget 40 | Budget 80 |
|----------|-----------|-----------|-----------|
| random | `14.8740 +/- 0.4055` | `14.9548 +/- 0.3373` | `15.0083 +/- 0.2379` |
| surrogate_only | `15.3272 +/- 0.0729` | `15.3272 +/- 0.0729` | `15.3350 +/- 0.0514` |
| causal | `15.3272 +/- 0.0729` | `15.3272 +/- 0.0729` | `15.3350 +/- 0.0514` |

### Runtime (mean seconds per run)

| Strategy | Budget 20 | Budget 40 | Budget 80 |
|----------|-----------|-----------|-----------|
| random | `199.0` | `332.2` | `608.5` |
| surrogate_only | `141.7` | `331.6` | `673.3` |
| causal | `131.7` | `383.0` | `634.3` |
...
|-----------|--------|----------|
| causal or surrogate_only beats random on test MAE | `FAIL` | Random achieved 105.14-105.21 vs 105.58-105.84 for causal/surrogate_only; random is marginally better at all budgets |
| Results hold across multiple seeds | `INCONCLUSIVE` | All comparisons are ties at 1% threshold across 5 seeds; differences are sub-0.7 MW |
| Gains not erased by validation-test gap | `INCONCLUSIVE` | Val-test gap is consistent (~14.9-15.3 MW) with no overfitting signal, but there are no gains to preserve |
| Chosen models not pathological on runtime | `PASS` | All strategies have comparable runtime; all runs completed without crashes |
...
| Results vary wildly across seeds | `NO` | Std across seeds is 0.00-0.36 MW, indicating high stability |
...
1. **causal and surrogate_only are identical.** This replicates the NORTH_C finding exactly. Both strategies produce identical results at every budget and seed combination. With Ax/BoTorch available, both strategies use Bayesian optimization but the causal graph prior does not alter the acquisition function enough to produce different suggestions. The causal focus variables do not meaningfully influence the optimization path.
...
5. **Larger validation-test gap than NORTH_C.** The val-test gap is approximately 14.9-15.3 MW for COAST vs 7.5-8.0 MW for NORTH_C. This is expected: COAST is a much larger load zone (7k-24k MW range vs 8.5k-17k MW range for NORTH_C), and the test window (May-Dec 2024) includes the high-demand summer peak which differs materially from validation (Oct 2023-May 2024).

6. **Random is marginally better with comparable runtime.** Random achieved ~0.4-0.7 MW lower test MAE with similar wall-clock time. With Ax/BoTorch available, engine-based strategies incur model-fitting overhead that offsets off-policy skip savings, making runtime comparable to random search.

7. **Very stable benchmark.** Standard deviations across seeds (0.00-0.36 MW) confirm that the benchmark harness and dataset are producing reproducible results, consistent with NORTH_C stability. Engine-based strategies show notably low variance (0.00-0.15 MW) due to deterministic Bayesian optimization paths.
...
| Best test MAE (random, B80) | 105.21 | 132.35 |
| Val-test gap | ~15.0-15.3 MW | ~7.5 MW |
...
| 3 | Investigate why causal focus variables do not alter Ax/BoTorch acquisition path | Agent | P1 |
