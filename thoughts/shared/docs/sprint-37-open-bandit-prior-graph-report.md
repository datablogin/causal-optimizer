...
| Engine flag | [`causal_optimizer/engine/loop.py`](../../causal_optimizer/engine/loop.py) `ExperimentEngine.__init__` (`pomis_minimal_focus: bool = False`) | Annotated as `bool` (mypy strict mode is the contract); threaded through both `suggest_next` paths (the main path and the MAP-Elites elite path). |
