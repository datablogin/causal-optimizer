...
5. **Null control has been clean for 11 synthetic runs across Sprints 18--29** (S26 did not re-run), confirming the optimizer does not manufacture false signal. Hillstrom and Criteo null controls also passed (Hillstrom with the caveat that permuted-outcome policy values can exceed the simple baseline; Criteo Run 1 and Run 2 within the 5% band on all cells). Total across synthetic, energy, and marketing domains: 14 passes.
